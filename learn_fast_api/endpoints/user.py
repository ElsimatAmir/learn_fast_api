...
@router.post("/getUserById/@{userId}", response_model=UserOutput)
async def GetUserById(userId: int) -> UserOutput:
    user = await UserRepository.getUserById(userId)
    return user


@router.post("/getUserByPhoneNumber/@{userPhoneNumber}", response_model=UserOutput)
async def GetUserById(userPhoneNumber: str) -> UserOutput:
    user = await UserRepository.getUserByPhoneNumber(userPhoneNumber)
    return user


@router.post("/getUserByEmail/@{userEmail}", response_model=UserOutput)
